...
void exchange_ghost_cells(int *matrix, int num_rows, int num_cols, int *neighbours)
{
  ...
  if (neighbours[2] != -1) {
    ...
    MPI_Type_create_subarray(2, array_of_sizes, array_of_subsizes, 
                             array_of_starts, MPI_ORDER_C, MPI_INT, &column_type);
    MPI_Type_commit(&column_type);
    MPI_Irecv(matrix, 1, column_type, neighbours[2], 0, MPI_COMM_WORLD, &reqs[r++]);
    MPI_Type_free(&column_type);
  }
  ...
  if (neighbours[3] != -1) {
    ...
    MPI_Type_create_subarray(2, array_of_sizes, array_of_subsizes, 
                             array_of_starts, MPI_ORDER_C, MPI_INT, &column_type);
    MPI_Type_commit(&column_type);
    MPI_Irecv(matrix, 1, column_type, neighbours[3], 0, MPI_COMM_WORLD, &reqs[r++]);
    MPI_Type_free(&column_type);
  }
  ...
  if (neighbours[2] != -1) {
    ...
    MPI_Type_create_subarray(2, array_of_sizes, array_of_subsizes, 
                             array_of_starts, MPI_ORDER_C, MPI_INT, &column_type);
    MPI_Type_commit(&column_type);
    MPI_Isend(matrix, 1, column_type, neighbours[2], 0, MPI_COMM_WORLD, &reqs[r++]);
    MPI_Type_free(&column_type);
  }
  ...
  if (neighbours[3] != -1) {
    ...
    MPI_Type_create_subarray(2, array_of_sizes, array_of_subsizes, 
                             array_of_starts, MPI_ORDER_C, MPI_INT, &column_type);
    MPI_Type_commit(&column_type);
    MPI_Isend(matrix, 1, column_type, neighbours[3], 0, MPI_COMM_WORLD, &reqs[r++]);
    MPI_Type_free(&column_type);
  }
